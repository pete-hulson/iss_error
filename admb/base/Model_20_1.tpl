...
//    2017 - Fishery length comp changed to 1 cm bins with plus bin of 45 cm
//         - 1984 & 1987 bottom trawl surveys removed
//         - Bottom trawl survey fit with lognormal
//         - Additional fishery selex block added post 2007 (coincides with CGOA rockfish program)
//    2020 - Changed priors on catchability (Jones et al) and natural mortality (Hamel 2015, Then et al. 2015)
//         - Construct fishery age comps with age-length key
...
// Read data from the control file
  !! ad_comm::change_datafile_name("goa_pop_2021.ctl");        // Read in phases, penalties and priors from "tem.ctl"
...
// Phases that general parameter estimation begins
  ...
  init_int             ph_mat                                  // Phase for estimating maturity
...
// Bottom trawl survey age composition data
  ...
  init_vector          iss_srv1_age(1,nyrs_srv1_age)        // input sample size
...
//Parameters to estimate maturity
  init_number          mat_a50(ph_mat)                         // age at 50% maturity
  init_number          mat_delta(ph_mat)                       // slope parameter for maturity
...
// Predicted Survey1 age comps, N, effn, sdnr, and offset       
  for (i=1;i<=nyrs_srv1_age;i++) {
   ...
   nmulti_srv1_age(i) = iss_srv1_age(i);
...
// Output file (.rep) which is loaded into R for data outputs
  ...
  report << 1 <<" "<<Like_C+Like_L  <<" " ; report << "Maturity Likelihood" <<endl;
  report << 1 <<" "<<sprpen  <<" " ; report << "Spawner-recruit penalty" <<endl;
