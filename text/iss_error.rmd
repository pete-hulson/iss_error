...
Compositional information on age and length comprise critical data products used statistical catch-at-age assessment models, as they facilitate the tracking of year classes and size-structure over time, thus improving our understanding of the population dynamics [@QuinnDeriso1999]. 
...
The weight assigned to annual compositions (the 'input sample size') can follow a myriad of methods (e.g., fixed values as in @Hulson2022b, number of samples or tows sampled upon as in @Hulson2021 or @Spencer2022, bootstrapping compositions as in @Stewart2014). 
...
For every fish species sampled for which age is capable of being determined from otoliths there is the potential for ageing error. 
...
Several methods have been developed to account for ageing error in statistical catch-at-age models when fitting age composition through the use of an ageing error matrix [e.g., @Candy2012; @Punt2008]. 
The essence of this approach is that the numbers-at-age estimated by the assessment model are 'corrected' through multiplication with an ageing error matrix, which assigns estimated numbers to adjacent age-classes depending on the magnitude of the ageing error within the specific age-class. 
...
CAAL data can be used directly within statistical catch-at-age models to inform estimates of growth as well as composition data [@Lee2019] and has been implemented in a number of stock assessments [e.g., @Mcgilliard2019; @Hulson2022b]. 
An inherent component to both the ALK and CAAL is the variability in length for a given age, in addition, when using CAAL data as an additional likelihood component to a statistical catch-at-age model one must determine the input sample size to be used to weight this information.
...
Using a current statistical catch-at-age model that is used for assessment and management as a case study we then show the effects of this added uncertainty on assessment model results.
